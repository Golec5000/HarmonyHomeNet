...
\begin{itemize}
    \item \textbf{Łańcuch resetowania haseł} -- Publiczny dostęp do punktów końcowych \texttt{/auth/forgot-password} i \texttt{/auth/reset-password} (\texttt{@Order(1)}).
...
%---
\noindent
Każdy łańcuch został skonfigurowany jako oddzielny komponent Spring Bean, co pozwala na łatwą modyfikację w zależności od potrzeb. Kluczowe elementy konfiguracji to:
\begin{itemize}
    ...
    \item Bezstanowe zarządzanie sesjami przy użyciu \texttt{SessionCreationPolicy.STATELESS}.
...
Proces autoryzacji i uwierzytelniania z wykorzystaniem tokenów JWT składa się z następujących etapów:
...
\begin{itemize}
    ...
    \item W przypadku nieprawidłowego tokenu lub jego braku użytkownik otrzymuje odpowiedź HTTP \texttt{403}.
...
Mechanizm zarządzania czarną listą tokenów w systemie „Harmony Home Net” zapewnia możliwość unieważnienia tokenów JWT, co jest kluczowe podczas wylogowywania użytkowników. Dzięki temu rozwiązaniu system natychmiast kończy sesję użytkownika, uniemożliwiając dalszy dostęp do chronionych zasobów przy użyciu unieważnionego tokenu.

\noindent \textbf{Proces wylogowania i dodawania tokenu do czarnej listy}\newline
Wylogowanie użytkownika odbywa się za pomocą dedykowanego łańcucha filtrów \texttt{logoutSecurityFilterChain}, zdefiniowanego w klasie \texttt{SecurityConfig}. Kod konfiguracji łańcucha wylogowania został przedstawiony poniżej.
...
W procesie wylogowania system wykonuje następujące kroki:
\begin{itemize}
...
Funkcje klasy \texttt{TokenBlacklistService} obejmują:
\begin{itemize}
    ...
    \item \texttt{clearExpiredTokens} -- usuwa wygasłe tokeny z czarnej listy, redukując wykorzystanie pamięci.
\end{itemize}
...
Mechanizm czarnej listy tokenów pozwala na skuteczne zabezpieczenie systemu przed dostępem nieautoryzowanym po wylogowaniu użytkownika lub w przypadku naruszeń bezpieczeństwa.


\subsection{Mechanizmy resetowania hasła}

Mechanizm resetowania hasła w systemie „Harmony Home Net” został zaprojektowany z uwzględnieniem podstawowych zasad bezpieczeństwa, jednak w uproszczonej formie w stosunku do pełnoprawnych wdrożeń produkcyjnych. Proces został podzielony na kilka etapów, które pokazano na rysunku~\ref{fig:ui_password_reset}.
...
Resetowanie hasła przebiega według następujących kroków:
\begin{itemize}
...
Obecna implementacja zapewnia podstawowe mechanizmy ochrony:
\begin{enumerate}
...
System zarządzania użytkownikami w prototypie \textbf{Harmony Home Net} został zaprojektowany jako zamknięty system, co oznacza, że rejestracja nowych użytkowników odbywa się wyłącznie za pośrednictwem pracowników administracyjnych. Takie podejście pozwala zwiększyć bezpieczeństwo, ograniczając dostęp do systemu jedynie do osób upoważnionych.
...
\textbf{Rola w autoryzacji użytkowników} -- dzięki metodzie \texttt{getAuthorities()} encja umożliwia odczytywanie ról przypisanych użytkownikowi. W połączeniu z mechanizmami Spring Security pozwala to na efektywne zarządzanie dostępem do zasobów systemu.
...
\textbf{Zasady działania hierarchii ról}:
\begin{itemize}
...
Poniżej przedstawiono kod definiujący hierarchię ról.
...
\begin{itemize}
    \item \textbf{Paginacja} -- mechanizm stronicowania umożliwia efektywne zarządzanie dużą liczbą użytkowników.
...
\textbf{Fragment kontrolera odpowiedzialnego za paginację i zarządzanie użytkownikami:}
...
\textbf{Potencjalne zastosowania ACL:}
\begin{itemize}
    \item Definiowanie uprawnień na poziomie obiektów, takich jak mieszkania czy zgłoszenia techniczne.
    \item Hierarchiczne zarządzanie uprawnieniami, dostosowane do struktury wspólnoty mieszkaniowej.
    \item Audytowanie i logowanie działań użytkowników, co pozwala monitorować potencjalne nadużycia.
\end{itemize}
...
Poniżej przedstawiono fragment kodu encji \texttt{Apartment}.
...
Tabela \texttt{PossessionHistory} pełni rolę pośredniczącą między użytkownikami a mieszkaniami. Pozwala zarządzać przypisaniami oraz przechowywać historię posiadania. Każda relacja użytkownik-mieszkanie jest unikalna, co zapewnia integralność danych.
...
\begin{itemize}
    \item \textbf{Istnienie użytkownika i mieszkania} -- Weryfikacja, czy użytkownik i mieszkanie istnieją w systemie.
    \item \textbf{Unikalność przypisania} -- Sprawdzenie, czy użytkownik nie ma już przypisanego danego mieszkania.
\end{itemize}

Fragment implementacji przypisywania właścicieli:
...
Planowane funkcje rozwojowe obejmują:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \texttt{userDocumentConnections} -- lista relacji dokumentu z użytkownikami.
\end{itemize}

\textbf{Encja \texttt{UserDocumentConnection}} -- zarządza relacją między dokumentami a użytkownikami mającymi do nich dostęp. Poniższy fragment przedstawia strukturę tej encji.
...
Przepływ dokumentów w systemie jest uproszczony i przedstawiony na schemacie~\ref{fig:document_schema}.
...
Schemat~\ref{fig:document_schema} ilustruje trzy główne przypadki:
\begin{itemize}
...
\textbf{Zarządzanie zgłoszeniami przez pracownika:}
\begin{itemize}
...
\textbf{Utrzymanie zgłoszeń przy zmianie właściciela mieszkania:}
\begin{itemize}
...
Serwis \texttt{ProblemReportServiceImp} obsługuje kluczowe operacje związane z zarządzaniem zgłoszeniami problemów.

\textbf{Tworzenie zgłoszenia problemu:}
\begin{itemize}
    \item Walidacja istnienia mieszkania oraz powiązania użytkownika z mieszkaniem.
    \item Utworzenie zgłoszenia z przypisaniem do odpowiedniego użytkownika i mieszkania.
    \item Zapis zgłoszenia w repozytorium.
\end{itemize}
...
\textbf{Aktualizacja zgłoszenia problemu:}
\begin{itemize}
    \item Zmiana szczegółów zgłoszenia i jego statusu.
    \item Automatyczne ustawienie daty zakończenia, jeśli status zmieni się na \texttt{DONE}.
    \item Powiadamianie użytkowników o zmianach statusu poprzez e-mail i SMS.
\end{itemize}
...
\begin{itemize}
    \item Integracja z systemami zarządzania usługami naprawczymi, aby automatycznie kierować zgłoszenia do odpowiednich serwisów.
...
\noindent Tabela pośrednicząca \texttt{AnnouncementApartment}
Zarządza relacjami między ogłoszeniami a mieszkaniami, umożliwiając przypisywanie i odłączanie ogłoszeń od konkretnych nieruchomości. 
...
\noindent Przypisywanie ogłoszeń do mieszkań
Funkcja \texttt{linkAnnouncementsToApartments} umożliwia przypisanie ogłoszenia do wybranych mieszkań. Proces obejmuje:
\begin{enumerate}
...
\noindent Odłączanie ogłoszeń od mieszkań
...
Forum składa się z dwóch głównych encji: \texttt{Topic} i \texttt{Post}. Encja \texttt{Topic} reprezentuje główny wątek dyskusyjny, natomiast encja \texttt{Post} odpowiada pojedynczym wypowiedziom w ramach tych wątków. Każda z encji jest powiązana z użytkownikami, umożliwiając przypisanie ich do twórców tematów oraz autorów postów.

\textbf{Encja \texttt{Topic}}  
Encja \texttt{Topic} przechowuje dane o wątku dyskusyjnym. Kluczowe pola to unikalny identyfikator, tytuł, data utworzenia oraz lista postów powiązanych z danym tematem. Przykładowy kod tej encji zawiera:
\begin{itemize}
...
\textbf{Encja \texttt{Post}}  
Encja \texttt{Post} reprezentuje wypowiedź użytkownika w ramach wątku. Przechowuje treść posta, datę utworzenia oraz odniesienia do powiązanego tematu i autora. Kluczowe pola to:
\begin{itemize}
...
System forum umożliwia realizację podstawowych operacji związanych z komunikacją użytkowników:
\begin{itemize}
...
Encje \texttt{Payment} oraz \texttt{PaymentComponent} odpowiadają za przechowywanie danych o płatnościach i ich szczegółach. Kluczowe cechy encji \texttt{Payment} obejmują:
\begin{itemize}
...
Kod encji \texttt{Payment} przedstawia poniższy fragment:
...
Encja \texttt{PaymentComponent} umożliwia rozbicie płatności na mniejsze części, np. za media. Kluczowe elementy to:
\begin{itemize}
...
W aktualnym prototypie proces zarządzania płatnościami jest uproszczony i składa się z trzech etapów:
\begin{enumerate}
...
W systemie za przypomnienia odpowiada klasa \texttt{PaymentReminderScheduler}. Mechanizm obejmuje:
\begin{itemize}
...
Harmonogram przypomnień pozwala na regularne powiadamianie użytkowników o statusie ich płatności, minimalizując ryzyko opóźnień. W przyszłości funkcjonalność ta może zostać rozwinięta o bardziej zaawansowane metody planowania i integrację z dodatkowymi kanałami komunikacji, co zostanie szczegółowo opisane w sekcji \textbf{„Systemy zewnętrzne”}.

\subsection{Profesjonalne podejście do zarządzania płatnościami}

W pełnowymiarowych systemach płatności zarządzanie odbywa się za pomocą zaawansowanych mechanizmów:
\begin{itemize}
    \item Automatyzacja generowania płatności na podstawie danych historycznych.
    \item Integracja z systemami zewnętrznymi, takimi jak Stripe, PayPal czy polski BLIK.
    \item Natychmiastowa synchronizacja danych finansowych z systemami księgowymi.
\end{itemize}

\subsection{Diagramy BPMN}

Diagramy BPMN umożliwiają graficzne modelowanie procesów płatniczych. Są one wykorzystywane do:
\begin{itemize}
    \item Definiowania ról i odpowiedzialności (np. użytkownik, administrator, zewnętrzny dostawca usług płatniczych).
    \item Opisu przepływów zdarzeń, takich jak inicjacja płatności czy wysyłka powiadomień.
    \item Analizy i optymalizacji procesów.
\end{itemize}
...
System głosowań w prototypie „Harmony Home Net” umożliwia organizację głosowań nad ważnymi decyzjami dla wspólnoty mieszkaniowej. Jego celem jest zapewnienie przejrzystości procesu decyzyjnego oraz wsparcie w zbieraniu głosów w formie elektronicznej.

\subsection{Obecny stan implementacji}

W aktualnym prototypie system obsługuje głosowania na poziomie administracyjnym. Proces obejmuje:
\begin{itemize}
...
\textbf{Tworzenie głosowania} odbywa się przy użyciu metody \texttt{createPoll}.
...
\textbf{Oddawanie głosów} przez właścicieli mieszkań realizowane jest poprzez metodę \texttt{vote}, która sprawdza zgodność danych i weryfikuje, czy dla danego mieszkania głos nie został już oddany:
...
W prototypie systemu zaimplementowano:
\begin{itemize}
    \item \textbf{Tworzenie głosowań} -- administratorzy mogą tworzyć głosowania z określeniem parametrów takich jak kworum czy minimalny próg procentowy.
    \item \textbf{Przypomnienia} -- mechanizm harmonogramowania przypomina użytkownikom o aktywnych głosowaniach za pomocą e-maila lub SMS.
    \item \textbf{Walidacja wyników} -- system weryfikuje spełnienie warunków kworum oraz progu procentowego.
\end{itemize}
...
Profesjonalny system zarządzania głosowaniami wymaga zaawansowanych funkcji, takich jak:
\begin{itemize}
...
Pełna implementacja wymagałaby wykorzystania zewnętrznych usług, takich jak Twilio lub Clickatell~\cite{twilio, clickatell}.

\subsection{System odczytów liczników mediów}

Odczyty liczników wody i prądu są generowane przez klasę \texttt{UtilityMeterServiceImp}. Obecnie wartości są losowo generowane raz na dobę.
...
Integracja z rzeczywistymi licznikami wymagałaby obsługi odpowiednich urządzeń i ich API.
...
Użytkownicy mogą wybrać preferowany sposób otrzymywania powiadomień dzięki encji \texttt{NotificationType}, obsługującej dwa rodzaje:
\begin{itemize}
    \item \texttt{EMAIL} -- wiadomości e-mail (działające).
    \item \texttt{SMS} -- wiadomości SMS (w trybie \emph{mock}).
\end{itemize}

Rozbudowa tej funkcji wymaga pełnej integracji z rzeczywistymi systemami komunikacyjnymi.
...
\begin{itemize}
    \item weryfikację poprawności logiki biznesowej serwisów,
    \item obsługę wyjątków, takich jak \texttt{AnnouncementNotFoundException}, \texttt{UserNotFoundException}, itp.,
    \item testy graniczne dla różnych wartości danych wejściowych.
\end{itemize}

\subsection{Przykład testów}

Przykładowe testy dla serwisu \texttt{AnnouncementService} sprawdzają poprawność działania jego metod, w tym obsługę wyjątków oraz operacje CRUD. Wybrane fragmenty kodu testów:
...
Wszystkie testy jednostkowe zakończyły się pozytywnie, co potwierdza, że zaimplementowane serwisy działają zgodnie z założeniami. Pokrycie testami obejmuje główne przypadki użycia aplikacji. Na rysunku~\ref{fig:test_presetation} przedstawiono wyniki dwóch serii testów.
